...
type ServerUnhealthyResponse struct {
	ResponseCodes ResponseCodeList `xml:"ResponseCode,omitempty"`
	UnknownNode   AnyList          `xml:",any"`
}

func ValidateServerUnhealthyResponse(v *ServerUnhealthyResponse, path string) []error {
	if v == nil {
		return nil
	}

	subPath := fmt.Sprintf("%s.ServerUnhealthyResponse", path)
